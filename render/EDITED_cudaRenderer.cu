...
CudaRenderer::CudaRenderer() {
    ...
    blockCircleIntersect = NULL;
    blockNumCircles = NULL;
    blockCircleUpdates = NULL;
}
...
CudaRenderer::~CudaRenderer() {
	...
	gpuErrchk(cudaFree(blockCircleIntersect));
	gpuErrchk(cudaFree(blockCircleUpdates));
	gpuErrchk(cudaFree(blockNumCircles));
    }
}
...
CudaRenderer::getImage() {
    ...
    gpuErrchk(cudaMemcpy(image->data,
               cudaDeviceImageData,
               sizeof(float) * 4 * imageWidth * imageHeight,
...
CudaRenderer::setup() {
    ...
    gpuErrchk(cudaMalloc(&cudaDeviceImageData, sizeof(float) * 4 * imageWidth * image->height));
    ...
    gpuErrchk(cudaMalloc(&blockCircleIntersect, sizeof(int) * nWidthBlocks * nHeightBlocks * nCirclesNextPow2));
    gpuErrchk(cudaDeviceSynchronize());
    gpuErrchk(cudaMalloc(&blockCircleUpdates, sizeof(int) * nWidthBlocks * nHeightBlocks * numCircles));
    gpuErrchk(cudaDeviceSynchronize());
    gpuErrchk(cudaMalloc(&blockNumCircles, sizeof(int) * nWidthBlocks * nHeightBlocks));
    gpuErrchk(cudaDeviceSynchronize());
    printf("GOODBYE %d, %d, %p\n", image->width, image->height, image);
    
    // cudaMalloc(&blockCircleIntersect, sizeof(int) * 64);

    /*

    printf("%d, %d --> %d, %d\n", image->width, image->height, nWidthBlocks, nHeightBlocks);
    printf("%d, %d --> %d, %d\n", image->width, image->height, nWidthBlocks, nHeightBlocks);
    // cudaMalloc(&blockCircleIntersect, sizeof(int) * nWidthBlocks * nHeightBlocks * nCirclesNextPow2);

    int* random;
    printf("hello! %d, %d, %p, %p, %p\n", image->width, image->height, image, blockCircleIntersect, random);
    // cudaMalloc(&random, sizeof(int) * 64);
    // cudaMalloc(&blockCircleIntersect, sizeof(int) * 64);
    cudaMalloc(&blockCircleUpdates, sizeof(int) * 64);
    printf("byebye! %d, %d, %p, %p, %p\n", image->width, image->height, image, blockCircleIntersect, random);

    printf("%d, %d --> %d, %d\n", image->width, image->height, nWidthBlocks, nHeightBlocks);
    cudaMalloc(&blockCircleUpdates, sizeof(int) * nWidthBlocks * nHeightBlocks * numCircles);
    printf("%d, %d --> %d, %d\n", image->width, image->height, nWidthBlocks, nHeightBlocks);
    cudaMalloc(&blockNumCircles, sizeof(int) * nWidthBlocks * nHeightBlocks);
    printf("%d, %d --> %d, %d\n", image->width, image->height, nWidthBlocks, nHeightBlocks);
    */
...
CudaRenderer::render() {
    cudaDeviceSynchronize();
    printf("cuda render (W=%d, H=%d, C=%d)\n", image->width, image->height, numCircles);
    printf("nWidthBlocks=%d, nHeightBlocks=%d\n", nWidthBlocks, nHeightBlocks);
    dim3 blockDim;
    dim3 gridDim;

    // Block x Circle intersection
    printf("step 1\n");
    blockDim = dim3(1, 1, 256);
    gridDim = dim3(nWidthBlocks, nHeightBlocks, (numCircles + 255)/256);
    kernelFindBlockCircleIntersections<<<gridDim, blockDim>>>(blockCircleIntersect, nCirclesNextPow2);
    cudaDeviceSynchronize();

    // Order circles that intersect block (Part1 - Exclusive Scan)
    printf("step 2.1\n");
    multiExclusiveScan(blockCircleIntersect, nWidthBlocks, nHeightBlocks, nCirclesNextPow2);
    cudaDeviceSynchronize();

    // Order circles that intersect block (Part2 - Find steps in Exclusive Scan)
    printf("step 2.2\n");
    blockDim = dim3(1, 1, 256);
    gridDim = dim3(nWidthBlocks, nHeightBlocks, (numCircles + 255)/256);
    kernelMultiFindStepLocs<<<gridDim, blockDim>>>(blockCircleIntersect, blockCircleUpdates,
		    blockNumCircles, nCirclesNextPow2, numCircles);
    cudaDeviceSynchronize();

    // Update pixels
    printf("step 3\n");
    blockDim = dim3(16, 16);
    gridDim = dim3(nWidthBlocks, nHeightBlocks);
    kernelPixelUpdate<<<gridDim, blockDim>>>(blockCircleUpdates, blockNumCircles);
    cudaDeviceSynchronize();
}
*/
